...
### Finding and filtering dashboards

When managing multiple dashboards, you can use filters on the dashboards list page to quickly find what you need:

- **Tag filter:** Select one or multiple tags to narrow down the dashboard list. Click the "Tags" button to open a searchable list of all available tags, then check the tags you want to filter by. The button shows a count of how many tags are currently selected.

- **Pinned filter:** Toggle this filter to show only dashboards you've pinned for quick access.

These filters work together, so you can combine tag filtering with the pinned filter to further narrow your results. Clear any active filters by clicking "Clear selection" in the tag menu or toggling off the pinned filter.
